...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const fetchJoke=()=>{  //function to retrieve the joke from the API
    ...
    xhr.onreadystatechange = function () { //onreadystatechange is a function that is called when the readyState property of the XMLHttpRequest object changes
        if (xhr.readyState === 4) { // if Request is done
            const divContent = document.getElementById('joke');  //getting the div element from the DOM
            if (xhr.status === 200) { // and if the request is successful
                try {
                    // Parse the JSON and extract the joke
                    const res = JSON.parse(xhr.responseText);
                    divContent.textContent = res.value; // display the extracted jokes
                } catch (error) { // if there is an error in the response
